...
- fix: the test runner reported `All tests passed` without running a single test on macOS. `run-tests.sh` timed each build with `date +%s%3N`, but `%N` (nanoseconds) is a GNU `date` extension — BSD `date` on macOS emits a literal `N` (`1780343287N`), which then poisoned the `$((end_time - start_time))` arithmetic. Under `set -euo pipefail` on the system bash 3.2, that arithmetic error broke out of the per-spec `for` loop and fell straight through to the final `echo "All tests passed"` with exit 0 — `wasmtime` never executed for any spec. Replaced the timer with a portable `now_ms` helper (`perl -MTime::HiRes`, falling back to `date +%s`-times-1000). (`run-tests.sh`)
- fix: a throw mid-expression (a throwing call in an argument list or variable initializer — `f(a, throws()); next()`, `const r = f(a, throws()); next()`) now actually short-circuits the rest of its block. The v1.1.1 entry below claimed this, but the mechanism was inert and three separate root causes kept it from ever firing: (1) `Globals.stmtStack` — the stack the `CallRef` reads to find its enclosing statement — was declared and reset but **never pushed to**, so `enclosingStmt`/`enclosingStmtContainer` were always null and the trailing unroll-check was never anchored; the linker now tracks the nearest enclosing statement-list member as it descends (`SourceLinker.visit`), gated by a precise `isStmtListMember` predicate so declaration *fragments* (`VariableDeclaration`, `FieldDeclaration`, …) that live in their own sub-arrays never get a guard spliced into them. (2) The expression-position branch in `CallRef.generate` guarded the anchor with `!isRefStatement(this.node, this.ref)`, which is true whenever the call's `ref` is any statement node — so it suppressed the guard for the exact case it was meant to handle (a throw in a `const r = …` initializer); replaced with a "is this call the *whole* statement?" check plus an idempotent `addUnrollCheckAfter` that dedupes, so the statement-position path and the argument path can't stack two identical guards. (3) The 2nd-and-later call site of an already-`visited` (clean) callee that received a throwing argument was orphaned: the `|| fnRef.visited` early return skipped the post-argument registration that pushes the `CallRef` into the enclosing function's `exceptions`, so although `smashStack` flagged the site it was never rewritten (no `isDefined`-if redirect, no trailing check) — `bump()` ran after the throw. The visit now skips only the redundant callee re-link, never the registration. (`transform/src/passes/source.ts`, `transform/src/types/callref.ts`, `transform/src/utils.ts`)
- fix: calls into a *nested* namespace function (`Outer.Inner.boom()`) are resolved and rewritten again. The v1.1.1 entry claimed this too, but the parent-side registration was never wired up: `visitNamespaceDeclaration` pushed every namespace (nested ones included) only into the flat source-level `local.namespaces` list and never into its parent's `.namespaces`, so `Outer.namespaces` was empty and `findLocalNs` dead-ended after the first path segment — the callee's `abort`/`throw` stayed raw and trapped the wasm module. Nested namespaces are now also pushed under their parent (`NamespaceRef.generate`'s double-reachability guard was already in place for exactly this). (`transform/src/passes/source.ts`)
- chore: the transform no longer prints anything during a normal build unless `DEBUG` is set. The `WRITE` dump block (`======WRITING======` / `Writing <file>`) and the `Found entry function …` line logged unconditionally — the former fired on every test build (which sets `WRITE` to emit `.tmp.ts` snapshots with `DEBUG=0`). Both are now gated behind `DEBUG > 0`; `WRITE` still writes its files, just silently. (`transform/src/index.ts`, `transform/src/passes/source.ts`)
- test: `arg-throws.spec.ts`, `call-pathing/call-pathing.spec.ts`, and `regression-bugs.spec.ts` (nested-namespace REGRESSION 7) now pass — they were silently skipped before the runner fix and then surfaced the regressions above. Full suite is 18/18 green.
