...
import { hasOnlyExceptions, removeExtension, replaceRef } from "./utils.js";
...
export class TryTransform extends Visitor {
  ...
  visitTryStatement(node, ref) {
    ...
    tryBlock = hasOnlyExceptions(node.bodyStatements)
  ...
  visitSource(node) {
    ...
    if (this.foundTry) {
      ...
      if (node.normalizedPath == WRITE) {
        writeFileSync(
          path.join(
            process.cwd(),
            this.baseDir,
            removeExtension(node.normalizedPath) + ".tmp.ts",
          ),
          toString(node),
        );
      }
...
//# sourceMappingURL=transform.old.js.map
