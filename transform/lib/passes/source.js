...
export class SourceLinker extends Visitor {
    ...
    visitFunctionDeclaration(node, isDefault = false, ref = null) {
        if (this.state != "gather")
            return this.linkFunctionRef(this.gatheredFns.find((v) => v.name == getFnName(node, this.path)));
    ...
    visitCallExpression(node, ref = null) {
        ...
        if (fnRef) {
            ...
            const externDec = this.source.imports.find((a) => a.declarations.find((b) => targetName == b.name.text || targetName.startsWith(b.name.text + ".")));
            if (externDec) {
