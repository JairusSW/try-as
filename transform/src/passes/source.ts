...
import { TryStatement } from "types:assemblyscript/src/ast";
...
export class SourceLinker extends Visitor {
  ...
  visitCallExpression(node: CallExpression, ref: Node | Node[] | null = null): void {
    ...
    // console.log(indent + "Looking for " + targetName, this.source.local.imports.length);
  ...
  static link(sources: Source[]): void {
    ...
    for (const [path, source] of SourceLinker.SS.sources) {
      if ([
        "assembly/test",
        "assembly/foo"
      ].includes(path)) console.log(path, source.functions, source.tries)
    }
  }
}